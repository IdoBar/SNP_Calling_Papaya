...
mamba install -n base libgcc gnutls libuuid readline cmake git tmux libgfortran parallel mamba gawk pigz rename genozip autoconf sshpass gh numpy scipy
...
Download the raw papaya whole-genome resequencing files (`.fastq.gz`) from NCBI SRA (WGS libraries from [PRJNA727683](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA727683/) and [UPDATE BIOPROJECT](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA727683/)).
...
# get WGS information from the SRA from project PRJNA693144
esearch -db sra -query PRJNA693144 | efetch -format runinfo  | gawk -F "," '$13=="WGS"' > Cpapaya_PRJNA693144_SRA_WGS_runinfo.csv

# get WGS information from the SRA from project PRJNA271489 
esearch -db sra -query PRJNA271489  | efetch -format runinfo  | gawk -F "," '$13=="WGS"' > Cpapaya_PRJNA271489_SRA_WGS_runinfo.csv

# get WGS information from the SRA from project PRJNA970517 
esearch -db sra -query PRJNA970517  | efetch -format runinfo  | gawk -F "," '$13=="WGS"' > Cpapaya_PRJNA970517_SRA_WGS_runinfo.csv

#combining SRAs from all four projects
cat Cpapaya_PRJNA*_SRA_WGS_runinfo.csv > Cpapaya_All_SRA_WGS_runinfo.csv
...
RUN_DIR="$WORK_DIR/FB_SNP_calling_21_08_2025"
...
# fields in SRA csv file: ID:{1},SM:{30},PL:{19},PM:{20}
cat $WORK_DIR/Cpapaya_All_SRA_WGS_runinfo.csv | parallel --csv --dry-run "bwa-mem2 mem -R \"@RG\tID:{1}\tSM:{30}\tLB:{30}\tPL:{19}\tPM:{20}\" -t \$[SLURM_CPUS_PER_TASK - 2] $REFERENCE.fasta.gz {1}_R1.trimmed.fastq.gz {1}_R2.trimmed.fastq.gz | bamsormadup tmpfile=\$TMPDIR/bamsormadup_\$(hostname)_\$SLURM_ARRAY_JOB_ID inputformat=sam threads=\$[SLURM_CPUS_PER_TASK - 2] indexfilename={1}.dedup.rg.csorted.bam.bai > {1}.dedup.rg.csorted.bam && mkdir -p {1}_bamqc &&  mosdepth -t \$SLURM_CPUS_PER_TASK -x -n {1}_bamqc/{1} {1}.dedup.rg.csorted.bam; unset DISPLAY; qualimap bamqc -bam {1}.dedup.rg.csorted.bam --java-mem-size=32G -c -gff $REFERENCE.bed -outdir {1}_bamqc && rm {1}.sra {1}_[12].fastq.gz"  > $RUN_DIR/$JOBNAME.cmds
...
find $BAM_DIR -name "*.bam" | xargs dirname | sort | uniq | parallel --dry-run "ls -1 {}/{/}.*.bam > {}/{/}.bamlist; bamtools merge -list {}/{/}.bamlist -out {}/{/}.bam; bamtools index -in {}/{/}.bam; rm {}/{/}.*.bam" > $RUN_DIR/$JOBNAME.cmds
