...
# create a folder for all the bam files and stardardise names
...
find $BAM_DIR -name "*.bam" | egrep -v "\.E[0-9]+\."  > $BAM_DIR/bam_list.txt
...
echo "~/bin/split_ref_by_bai_datasize.py -s 1e10 -r $GENOME.fasta.fai -L $BAM_DIR/bam_list.txt > $RUN_DIR/Sunup_target_regions_1e9.tsv" > $JOBNAME.cmds
# submit the job 
...
cut -f1 $GENOME.fasta.fai | parallel --dry-run "freebayes-parallel <(grep '{}' $RUN_DIR/Sunup_target_regions_1e9.tsv | gawk '{printf \"%s:%s-%s\n\", \$1, \$2, \$3}') \$SLURM_CPUS_PER_TASK -f $GENOME.fasta --genotype-qualities -g 200000 -C $MIN_DP -p $PLOIDY -L $BAM_DIR/bam_list.txt > $RUN_DIR/FB_array_output/{}.combined.vcf" > $RUN_DIR/$JOBNAME.cmds
...
echo "cat $RUN_DIR/FB_array_output/GWHBFSC*.combined.vcf | vcffirstheader | vcfstreamsort -w 1000 | vcfuniq > Cpapaya_Murdoch_WGRS_SRA_Sunup.bwa2.fb.diploid.vcf && bgzip Cpapaya_Murdoch_WGRS_SRA_Sunup.bwa2.fb.diploid.vcf && tabix Cpapaya_Murdoch_WGRS_SRA_Sunup.bwa2.fb.diploid.vcf.gz" > $JOBNAME.cmds
